...
app.get('/delete/:id', (req, res) => {
    ...
    db.inventory.remove({_id: mongojs.ObjectId(req.params.id)}, (err, doc) => {
        if (err) {
            res.send(err);
        } else {
...
app.post('/create', (req, res) => {
    db.inventory.insertOne({ "item": req.body.item, "qty": req.body.qty, "size":JSON.parse(req.body.size), "status": req.body.status },(err,doc)=>
    {
        if (err) {
            res.send(err)
        } else {
